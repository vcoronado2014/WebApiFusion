            ...
            try
            {
                ...
                if (triceles != null && triceles.Count > 0)
                {
                    foreach (VCFramework.Entidad.Tricel tri in triceles)
                    {
                        ...
                        us.OtroUno = DateTime.Parse(tri.FechaInicio.ToShortDateString(), culture).ToShortDateString() ;
                        us.OtroDos = DateTime.Parse(tri.FechaTermino.ToShortDateString(), culture).ToShortDateString();
